...
// Geometry
gl_vertex_t TurnLeftSegment::vertices[TURN_NUM_VERTICES];
GLushort TurnLeftSegment::indices[TURN_NUM_INDICES];

// Flag that indicates if the geometry has been defined
bool TurnLeftSegment::geometry_defined = false;

TurnLeftSegment::TurnLeftSegment(const glm::vec3& position, float orientation,
        int input):
...
TurnLeftSegment::~TurnLeftSegment()
{}
...
*/
void
TurnLeftSegment::get_output(int output, glm::vec3& pos, float& orient) const
{
    pos[0] = position[0]
        + SEGMENT_R/2 * (cos(orientation) + cos(orientation + M_PI/2));
    pos[1] = position[1]
        + SEGMENT_R/2 * (sin(orientation) + sin(orientation + M_PI/2));
    pos[2] = position[2];
...
void
TurnLeftSegment::init_geometry_static()
{
    float a, cosa, sina;
    ...
    for (int i = 0; i <= NTRIANGLES; i++) {
        ...
        a = (M_PI/2) * float(i) / float(NTRIANGLES);
        cosa = cos(a);
        sina = sin(a);

        // Perimeter vertices of the triangla fan (road)
